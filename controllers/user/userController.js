    ...
    try{ 
       ...
       // const categoryIds=categories.map((category)=>category._id.toString())
        const products=await Product.find({isListed:true,
             quantity:{$gt:0}
        }).sort({createdOn:-1}).populate('category')
...
console.log(products)
        ...
        res.render("home",{user:userData,name:req.session.user.name})
...
try { console.log("User inside login");
    ...
    if (!findUser.password) {
        return res.render("login", { message: "No password set. Please login with Google or reset your password." });
    }
...
try {
    ...
    if(findUser){
        ...
        res.render("forgot-password",{message:"User with this email does not exists"})
    }
...
const productDetails=async(req,res)=>{
    try {console.log("inside detail page")
        const userId=req.session.user;
        console.log(userId,"userid in product data starttedd first in product details");
        if(userId){
        ...
        const totalProducts = await Product.find({ category: findCategory._id }).populate('category');
         const totalProcount=   await Product.countDocuments({ category: findCategory._id }).populate('category');
